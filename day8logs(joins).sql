...
MariaDB [csd2204s18]> select products.code,product.name,manu.name
    -> from manu
    -> inner join products
...
MariaDB [csd2204s18]> select count(*), name
    ->     from products as p
    ->     inner join manu m
    ->     on m.code=p.manufacturer
    ->     group by manufacturer;
...
MariaDB [csd2204s18]> select count(*), name
    ...
    ->     group by manufacturer;
ERROR 1052 (23000): Column 'name' in field list is ambiguous
MariaDB [csd2204s18]> select count(*), p.name
    ->     from products as p
    ->     inner join manu m
    ->     on m.code=p.manufacturer
    ->     group by manufacturer;
...
MariaDB [csd2204s18]> select manufacturer, m.name,count(*) "No.of Items produced"
    ->     from products as p
    ->     inner join manu m
    ->     on m.code=p.manufacturer where No.of Item produced>3 group by manufacturer;
ERROR 1064 (42000): You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'Item produced>3 group by manufacturer' at line 4
MariaDB [csd2204s18]> select manufacturer, m.name,count(*) "No.of Items produced"
    ->     from products as p
    ->     inner join manu m
    ->     on m.code=p.manufacturer where count(*)>2 group by manufacturer;
...
MariaDB [csd2204s18]> select manufacturer, m.name,count(*) as C
    ->     from products as p
    ->     inner join manu m
    ->     on m.code=p.manufacturer where C>2 group by manufacturer;
ERROR 1054 (42S22): Unknown column 'C' in 'where clause'
MariaDB [csd2204s18]> select manufacturer, m.name,count(*) "C"
    ->     from products as p
    ->     inner join manu m
    ->     on m.code=p.manufacturer where C>2 group by manufacturer;
ERROR 1054 (42S22): Unknown column 'C' in 'where clause'
...
MariaDB [csd2204s18]> select products.code, products.name, manu.name from products left join manu on products.code=manu.code
    -> union
    -> select products.code, products.name, manu.name from products left join manu on products.code=manu.code;
+------+-----------------+-------------------+
| code | name            | name              |
+------+-----------------+-------------------+
|    1 | Hard drive      | Sony              |
...
MariaDB [csd2204s18]> select p1.code,p1.name,p2.name from products p1, productsp2 where p.code<>p2.code;
ERROR 1146 (42S02): Table 'csd2204s18.productsp2' doesn't exist
MariaDB [csd2204s18]> select p1.code,p1.name,p2.name from products p1, products p2 where p.code<>p2.code;
ERROR 1054 (42S22): Unknown column 'p.code' in 'where clause'
